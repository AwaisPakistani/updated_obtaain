...
class FrontJournalController extends Controller
{
    ...
    public function front_register(Request $request,$journal_id){
        ...
        if($request->isMethod('post')){
            ...
            $frontuser->first_name=$data['full_name'];
            $frontuser->last_name='';
    ...
    public function author_dashboard($journal_id){
        ...
        $papers=Paper::where(['frontuser_id'=>$id,'journal_id'=>$journal_id,'percentagePaper'=>100])->where('status','!=','published')->orderBy('id', 'DESC')->get();
        //$journal=Journal::where('assign_chiefeditor',$chief)->get();
        
        $paper_need = Paper::where(['frontuser_id'=>$id,'journal_id'=>$journal_id,'percentagePaper'=>100,'status'=>'author','revision_status'=>1])->where('status','!=','published')->get();

        $paper_decisioned = Paper::where(['frontuser_id'=>$id,'journal_id'=>$journal_id,'percentagePaper'=>100,'status'=>'author'])->where('status','!=','published')->get();
        //dd($journal);
        $journal=Journal::where('id',$journal_id)->first();
        ...
        $declined = Paper::where(['frontuser_id'=>$id,'journal_id'=>$journal_id,'percentagePaper'=>100,'status'=>'author','revision_status'=>'declined'])->where('status','!=','published')->get();

        $production = Paper::where(['frontuser_id'=>$id,'journal_id'=>$journal_id,'percentagePaper'=>100,'status'=>'publisher','revision_status'=>'accepted'])->where('status','!=','published')->get();
        $revisions_being_processed = Paper::where(['frontuser_id'=>$id,'journal_id'=>$journal_id,'percentagePaper'=>100,'status'=>'chief'])->where('status','!=','published')->get();
   ...
   public function front_chiefpapers_assign($paper_id){
        ...
        )->where('journal_id',$paper->journal_id)->get();
        //dd($reviewers);
   ...
   public function review_submission_stage1(Request $request,$journal_id,$paper_id){
        ...
        $author = Auth::guard('frontuser')->user()->id;
        $papers=Paper::where(['frontuser_id'=>$author,'journal_id'=>$journal_id])->where('status','!=','published')->get();

        $paper_need = Paper::where(['frontuser_id'=>$author,'journal_id'=>$journal_id,'percentagePaper'=>100,'status'=>'author','revision_status'=>1])->where('status','!=','published')->get();

        $paper_decisioned = Paper::where(['frontuser_id'=>$author,'journal_id'=>$journal_id,'percentagePaper'=>100,'status'=>'author'])->where('status','!=','published')->get();
        ...
        $revision_backToAuthor = Paper::where(['frontuser_id'=>$author,'journal_id'=>$journal_id,'percentagePaper'=>100,'status'=>'author'])->where(function($query) {
                $query->where('revision','>',1);
          })->where('status','!=','published')->where('status','!=','published')->get();
        $declined = Paper::where(['frontuser_id'=>$author,'journal_id'=>$journal_id,'percentagePaper'=>100,'status'=>'author','revision_status'=>'declined'])->where('status','!=','published')->get();
        ...
        $revisions_being_processed = Paper::where(['frontuser_id'=>$author,'journal_id'=>$journal_id,'percentagePaper'=>100,'status'=>'chief'])->where('status','!=','published')->get();
        //$files = File::where('paper_id',$paper_id)->get();
   ...
   public function author_paper_files($journal_id,$paper_id){
        ...
        $production = Paper::where(['frontuser_id'=>$author,'journal_id'=>$journal_id,'percentagePaper'=>100,'status'=>'publisher','revision_status'=>'accepted'])->where('status','!=','published')->get();
        $revisions_being_processed = Paper::where(['frontuser_id'=>$author,'journal_id'=>$journal_id,'percentagePaper'=>100,'status'=>'chief'])->where('status','!=','published')->get();
        return view('front.pages.journal.author.files',compact('journal','paper','papers','files','paper_need','revision_backToAuthor','declined','paper_decisioned','production','revisions_being_processed','paper_id'));
   }
    ...
    public function chief_paper_report($journal_id,$paper_id){
        ...
        //dd($paper_report);
        return view('front.pages.journal.chiefeditor.paper_report',compact('journal','paper_report','paper_report_reviewer2','paper_id'));
   }
   ...
   public function author_submission_needing_revisions($journal_id){
          ...
          $papers=Paper::where(['frontuser_id'=>$myid,'journal_id'=>$journal_id,'percentagePaper'=>100,'status'=>'author'])->orderBy('id', 'DESC')->where('status','!=','published')->get();
          ...
          // })->get();
          $paper_need = Paper::where(['frontuser_id'=>$myid,'journal_id'=>$journal_id,'percentagePaper'=>100,'status'=>'author','revision_status'=>1])->where('status','!=','published')->get();

         $paper_decisioned = Paper::where(['frontuser_id'=>$myid,'journal_id'=>$journal_id,'percentagePaper'=>100,'status'=>'author'])->where('status','!=','published')->get();
          ...
          $production = Paper::where(['frontuser_id'=>$myid,'journal_id'=>$journal_id,'percentagePaper'=>100,'status'=>'publisher','revision_status'=>'accepted'])->where('status','!=','published')->get();
          $revisions_being_processed = Paper::where(['frontuser_id'=>$myid,'journal_id'=>$journal_id,'percentagePaper'=>100,'status'=>'chief'])->where('status','!=','published')->get();
          //dd($reports);
   ...
   public function papers_decisioned($journal_id){
        $myid = Auth::guard('frontuser')->user()->id;
        $journal = Journal::where('id',$journal_id)->first();
        $papers=Paper::where(['frontuser_id'=>$myid,'journal_id'=>$journal_id,'percentagePaper'=>100])->orderBy('id', 'DESC')->where('status','!=','published')->get();
        $paper_need = Paper::where(['frontuser_id'=>$myid,'journal_id'=>$journal_id,'percentagePaper'=>100,'status'=>'author','revision_status'=>1])->where('status','!=','published')->get();
        ...
        $declined = Paper::where(['frontuser_id'=>$myid,'journal_id'=>$journal_id,'percentagePaper'=>100,'status'=>'author','revision_status'=>'declined'])->where('status','!=','published')->get();
   ...
   // revisions_being_processed
    public function revisions_being_processed($journal_id){
        ...
        $journal = Journal::where('id',$journal_id)->first();
        $papers=Paper::where(['frontuser_id'=>$myid,'journal_id'=>$journal_id,'percentagePaper'=>100])->orderBy('id', 'DESC')->where('status','!=','published')->get();
        $paper_need = Paper::where(['frontuser_id'=>$myid,'journal_id'=>$journal_id,'percentagePaper'=>100,'status'=>'author','revision_status'=>1])->where('status','!=','published')->where('status','!=','published')->get();
   ...
   public function papers_journal_issues($issue_id){
     ...
     return view('front.pages.journal.publish.index',compact('issue','published','issue_id'));
   }
   ...
   public function chiefSendPaperToPublisherDirectly($paper_id){
     $paper   = Paper::where('id',$paper_id)->update([
       'status'=>'publisher'
     ]);

     Session::flash('success_message','Paper has successfully send to publisher');
     return redirect()->back();


   }

   public function pin_journal($journal_id){
    $journal = Journal::where('id',$journal_id)->select('journal_name')->first();
    
    $pinned = DB::table('pinnned_journals')->where('journal_id',$journal_id)->count();
    
    if($pinned > 0){
        Session::flash('error_message','Already pinned');
        return redirect()->back();
    }
    DB::table('pinnned_journals')->insert([
      'journal_id'=>$journal_id,
      'journal_name'=>$journal->journal_name,
      'pinned_at'=>NOW()
    ]);
    Session::flash('success_message','Journal Pinnded successfully');
    return redirect()->back();
   }

   public function author_delete_file($paper_id,$file_id){
   ...
   public function author_addnewfile(Request $request){
        $data = $request->all();
        //dd($data);
        $image_path = $request->file('file')->store('papers', 'public');
        ...
        Session::flash('success_message','File added successfully');
        return redirect()->back();
   }
